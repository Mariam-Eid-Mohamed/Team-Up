...
const CreateTeamModal: React.FC<CreateTeamModalProps> = ({ isOpen, onClose, courseworkId }) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setError(
        err.response?.data?.message || "An unexpected error occurred while creating the team."
      );
      setIsSubmitting(false);
    }
  };
      ...
      <div className="bg-white w-full max-w-md rounded-2xl shadow-2xl p-6 relative animate-in fade-in zoom-in duration-200">
        ...
        <div className="space-y-4 text-center mb-10">
          ...
          <input
            ...
            className={`w-full text-center py-3 px-4 border-b-2 outline-none text-lg transition-colors placeholder:text-gray-300 ${error ? "border-red-500" : "border-gray-100 focus:border-[#2D7A78]"
              }`}
          />
