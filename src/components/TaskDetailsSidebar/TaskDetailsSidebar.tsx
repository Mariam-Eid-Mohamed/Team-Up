...
import { Edit2, Trash2, UploadCloud, X, Download, Info, Loader2 } from "lucide-react";
import { getTaskDetails, uploadDeliverable } from "@/Services/Task Endpoints/Endpoints";
...
export default function TaskDetailsSidebar({
  ...
  const creatorName = taskDetails.creator
    ? `${taskDetails.creator.firstName || ""} ${taskDetails.creator.lastName || ""}`.trim() || "Unknown"
    : "Unknown";

  const assigneeName = taskDetails.assignee
    ? `${taskDetails.assignee.firstName || ""} ${taskDetails.assignee.lastName || ""}`.trim() || "Unassigned"
    : "Unassigned";

  const isAssignee = taskDetails.assignee && taskDetails.assignee.id === userId;
  const hasUploadedFile = !!(taskDetails.deliverableFileUrl || taskDetails.deliverable_file_url);
  ...
  const handleAssigneeChange = (newAssigneeName: string) => {
    ...
    setTaskDetails((prev: any) => ({
      ...prev,
      assignee: newAssigneeName === "Unassigned" ? null : {
        ...prev.assignee,
        firstName,
        lastName,
        profilePicture: null
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files.length > 0) {
      ...
      const requiredType = taskDetails.deliverableType || taskDetails.deliverable_type;
      
      if (requiredType && !file.name.toLowerCase().endsWith(requiredType.toLowerCase())) {
        toast.error(`Wrong file type. This task requires a ${requiredType} file.`);
        return;
      }
  ...
  const deliverableType = taskDetails.deliverableType || taskDetails.deliverable_type || ".pdf";
  const deliverableFileUrl = taskDetails.deliverableFileUrl || taskDetails.deliverable_file_url;
  const markedAsDoneAt = taskDetails.markedAsDoneAt || taskDetails.marked_as_done_at || taskDetails.updatedAt;
    ...
    <>
      ...
      <div className="fixed right-0 top-0 h-screen w-full max-w-[500px] bg-white z-[1000] shadow-2xl flex flex-col font-sans border-l border-gray-100 animate-in slide-in-from-right duration-200">
        ...
        <div className="flex items-center justify-between px-8 pt-8 pb-4">
          <h2 className="text-3xl font-bold text-gray-900 tracking-tight">{taskDetails.name}</h2>
        ...
        <div className="flex-1 overflow-y-scroll px-8 py-6 space-y-6 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
          ...
          <div className="flex items-center justify-between text-[15px]">
            ...
            <span className="text-gray-700 font-medium">
              {taskDetails.deadline ? new Date(taskDetails.deadline).toLocaleDateString() : ""}
            </span>
          </div>
              ...
              <div className="flex items-center gap-2.5">
                <div className="w-7 h-7 rounded-full border border-gray-200 overflow-hidden bg-gray-50 flex items-center justify-center">
                  {taskDetails.creator?.profilePicture ? (
                    <img src={taskDetails.creator.profilePicture} alt="" className="w-full h-full object-cover" />
                  ) : (
                ...
                >
                  <div className="w-5 h-5 rounded-full border border-gray-200 overflow-hidden bg-gray-50 flex items-center justify-center">
                    {taskDetails.assignee?.profilePicture ? (
                      <img src={taskDetails.assignee.profilePicture} alt="" className="w-full h-full object-cover" />
                    ) : (
                  ...
                  <span className="text-gray-800 font-medium text-sm">{assigneeName}</span>
            ...
            {hasUploadedFile && !isReplacing ? (
              <div className="space-y-3">
                <div className="flex items-center justify-between border border-emerald-100/70 rounded-lg px-4 py-3 bg-emerald-50/20">
                  <div className="space-y-0.5">
                    <p className="text-sm font-medium text-gray-800 truncate max-w-[200px]" title={getFileName(deliverableFileUrl)}>
                      {getFileName(deliverableFileUrl)}
                    </p>
                    <p className="text-xs text-gray-400">
                      {markedAsDoneAt ? `Uploaded on ${new Date(markedAsDoneAt).toLocaleString()}` : "Uploaded"}
                    </p>
                  </div>
              ...
              <div className="border border-gray-200 rounded-xl p-5 bg-white flex flex-col items-center justify-center shadow-xs">
                ...
                >
                  ...
                  <p className="text-xs font-semibold text-teal-600 hover:underline">click to browse</p>
                  <p className="text-[11px] text-gray-400 mt-0.5">Requires {deliverableType} file</p>
                </div>
              ...
              <div className="flex items-start gap-2.5 border border-sky-100 bg-sky-50/40 p-3 rounded-lg text-xs text-sky-700 leading-normal">
                <Info size={15} className="mt-0.5 flex-shrink-0" />
                <p>Only the assigned member can upload deliverables for this task.</p>
              </div>
            )}
          </div>
