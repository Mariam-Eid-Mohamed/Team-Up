...
import { getTeamDetails, lockTeam } from "../../../Services/team Endpoints/Endpoints";
...
export default function TeamWorkspace() {
  ...
  const handleInviteClick = () => {
    ...
    navigate(`/${role}/classes/${teamData.classId || 'unknown'}/available-members`);
  };
  ...
  const isLeader = teamData?.teamMembers?.some((m: any) => m.id === userId && m.role === 'LEADER');
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-[#F9FAFB]">
        <div className="text-center">
          <p className="text-red-500 font-medium mb-4">{error || "Team not found"}</p>
          <button
            onClick={() => navigate(-1)}
    ...
    <div className="max-w-6xl mx-auto p-4 md:p-6 bg-[#F9FAFB] min-h-screen">
      ...
      {activeTab === "Members" && (
        <div className="space-y-6 md:space-y-8">
          {/* Instructors Section */}
          <section>
            <h2 className="text-[#1B4D49] font-bold text-lg mb-4">Instructor</h2>
            <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-3 md:p-4 flex items-center justify-between">
              <div className="flex items-center gap-3 md:gap-4">
                <div
                  ...
                  style={{ backgroundColor: teamData.classColor || '#f3f4f6', color: teamData.classColor ? '#ffffff' : '#6b7280' }}
                >
                  {teamData.instructor ? teamData.instructor.charAt(0) : "?"}
                </div>
                <div>
                  <p className="font-bold text-sm md:text-base text-gray-800">{teamData.instructor || "No Instructor Assigned"}</p>
